...
type UseSidebarControllerProps = {
	defaultOpen?: boolean;
	open?: boolean;
	onOpenChange?: (open: boolean) => void;
	cookieName: string;
	cookieMaxAge: number;
	keyboardShortcut: string; // e.g. "b"
};
...
export function useSidebarController({
	...
	cookieMaxAge,
	keyboardShortcut,
}: UseSidebarControllerProps): SidebarContextProps {
	...
	const setOpen = React.useCallback(
		(value: boolean | ((prev: boolean) => boolean)) => {
			...
			// Persist (client-only)
			document.cookie = `${cookieName}=${next}; path=/; max-age=${cookieMaxAge}`;
		},
		[cookieName, cookieMaxAge, onOpenChange, open]
	);
	...
	React.useEffect(() => {
		const onKeyDown = (event: KeyboardEvent) => {
			const isHotkey = event.key === keyboardShortcut && (event.metaKey || event.ctrlKey);
			if (!isHotkey) return;
...
function SidebarProvider({
	...
	const contextValue = useSidebarController({
		defaultOpen,
		open: openProp,
		onOpenChange: setOpenProp,
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
	...
	return (
		<Button
			...
			{...props}
		>
			{open ? <IconLayoutSidebarLeftCollapse className="size-icon" /> : <IconLayoutSidebarLeftExpand className="size-icon" />}
