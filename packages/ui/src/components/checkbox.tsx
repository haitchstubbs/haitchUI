...
type CheckboxProps = Omit<
  ...
  "defaultChecked" | "checked" | "onChange" | "type"
> & {
  /**
  ...
  return (
    <span data-slot="checkbox-root" className="inline-flex items-center">
      ...
      <button
        ...
        className={cn(
          // Base styling
          "border-input dark:bg-input/30 size-4 shrink-0 rounded-[4px] border shadow-xs transition-shadow outline-none",
      ...
      >
        <span data-slot="checkbox-indicator" className="grid place-content-center text-current transition-none">
          {isChecked ? <IconCheck className="size-3.5" /> : null}
        </span>
      </button>
    </span>
  );
