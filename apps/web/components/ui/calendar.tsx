...
function Calendar({
	...
	const monthShort = React.useMemo(() => new Intl.DateTimeFormat("en-US", { month: "short", timeZone: "UTC" }), []);
	return (
		<DayPicker
			...
			formatters={{
				formatMonthDropdown: (d) => {
					// Force a date that cannot fall into the previous month in UTC
					const safe = new Date(Date.UTC(d.getFullYear(), d.getMonth(), 15));
					return monthShort.format(safe);
				},
				...formatters,
			}}
			classNames={{
